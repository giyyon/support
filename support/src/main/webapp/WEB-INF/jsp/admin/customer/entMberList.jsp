...
<%@ include file="../../include/taglib.jsp" %>
        	...
        	<div id="content-group">
            	...
            	<div id="contents">
                    ...
                    <ul class="tap4_wrp mt20">
	                    <li><a href= "#"  onclick="javascript:getBsnsSeList('') "    <c:if test="${userSearchVO.searchBsnsSe == '' }" > class="select"</c:if>>전체기업회원</a></li>
                    ...
                    <div class="srh_wrap tlr mtf14">
                    	<ul class="lst_m">
                            <li class="minfo"><a href="/admin/entMberDetail.do">비회원등록</a></li>
                        </ul>
                            ...
                            <form id="subForm"    method="post"  action="${contextPath}/admin/gnrMberDetail.do" >
            			     ...
            			     <c:forEach var="result" items="${resultList}" varStatus="status">
					          <tr>
	                                ...
	                                <td><div class="photo">
	                              								 <c:choose>
				                                        			<c:when test= "${result.atchImgLogoFileId ==null || result.atchImgLogoFileId ==''}">
					                                        			<img src="${contextPath}/img/noimg.png">
					                                        		</c:when>
					                                        		<c:otherwise>		
					                                        			<img src="${contextPath}/files/imageThumnailSrcByFileId.do?fileId=${result.atchImgLogoFileId}"  style="height: auto; 0% 50% no-repeat;">		                                        		
	<%-- 				                                        			<dt class="img_div"  style="background: url(${contextPath}/files/imageThumnailSrcByFileId.do?fileId=${result.atchImgMainFileId}) 0% 50% no-repeat;"></dt> --%>
					                                        		</c:otherwise>
					                                        	</c:choose>
	                                ...
	                                <td>
	                                    <c:out value='${result.entrprsmberId}'/><br><span class="mem"><img src="${contextPath}/img/admin/ico_mem_ok.png" alt="정회원">
	                                    <c:choose>
	                                    	<c:when test="${result.mberType == 'D01'}">비</c:when>
	                                    	<c:otherwise>정</c:otherwise>
	                                    </c:choose>_<c:out value='${result.entrprsMberSeNm}'/></span>
	                                </td>
	                                <td class="txt11_space"><c:out value='${result.applcntNm}'/><br><c:out value='${result.establishYear}'/>-<c:out value='${result.establishMonth}'/>-<c:out value='${result.establishDay}'/></td>
	                                <td class="num2"><c:out value='${result.mbtlnum}'/><br><a href="#"><c:out value='${result.applcntEmailAdres}'/></a></td>
	                                <td class="num2"><c:out value='${result.reprsntAreaNo}'/>-<c:out value='${result.reprsntEntrprsMiddleTelno}'/>-<c:out value='${result.reprsntEntrprsEndTelno}'/><br><c:out value='${result.fxnum}'/></td>
	                                <td><c:out value='${result.cmpnyNm}'/><br><a href="#" class="tip_long"><c:out value='${fn:substring(result.adres,0,7)}'/> <p><c:out value='${result.adres}'/>&nbsp; <c:out value='${result.detailAdres}'/></p></td>
	                                <td class="num2"><c:out value='${result.sbscrbDe}'/><br><c:out value='${result.updtDe}'/></td>
	                                <td class="m_ch">
	                                <p onclick='goMberDetail("${result.entrprsmberId}")'  style="cursor:pointer"><img src="${contextPath}/img/admin/ico_detail.png" alt="회원상세정보" >  회원상세정보</p>
	                                <a href="#" class="btn-example" onclick="layer_open('layer2', '<c:out value='${result.entrprsmberId}'/>', '<c:out value='${result.uniqId}' />');return false;"><img src="${contextPath}/img/admin/ico_pw.png" alt="비밀번호변경"> 비밀번호변경</a></td>
								...
								<div class="pop-conts">
									...
									<div class="layerContent">
										<form name = "passwordForm" >
											<input type="hidden" value="111111" name="entrprsMberPassword">
											<input type="hidden" name="entrprsmberId">
	...
	$(function(){
		...
		$("p[orderType]").bind("click", function(){
			...
			$("#frm").attr({action:"${contextPath}/admin/entMberList.do", target:""});
			$("#frm").submit();
		});
	...
	function getBsnsSeList(bsnsSe){
		...
		$('#frm').attr('action', '${contextPath}/admin/entMberList.do'); 
		$("#frm").submit();
	}
	...
	// 상세조회 이동
	function goMberDetail(entrprsmberId){
		entrprsmberId
		$("#frm input[name=entrprsmberId]").val(entrprsmberId);
		$('#frm').attr('action', '${contextPath}/admin/entMberDetail.do'); 
		$("#frm").submit();
	}
...
<script type="text/javascript">
	function layer_open(el, targetId, targetUniqId){
		...
		temp.find('#passwordBtn').bind('click', function() {
			doSubmit();  
		  });
		var doSubmit = function(){
			temp.find('#passwordBtn').unbind('click');
